// Wilmington Dashboard JavaScript - Based on Reddot structure
class WilmingtonDashboard {
    constructor() {
        this.charts = {};
    ...
    waitForData() {
        return new Promise((resolve) => {
            const checkData = () => {
                if (wilmingtonDashboardData && wilmingtonDashboardData.processedData) {
                    resolve();
                } else {
    ...
    updateHeaderMetrics() {
        const stats = wilmingtonDashboardData.getPlatformStats();
        const contentMetrics = wilmingtonDashboardData.getContentMetrics();
    ...
    loadMetricsSummary() {
        ...
        const contentMetrics = wilmingtonDashboardData.getContentMetrics();
        const topPlatforms = wilmingtonDashboardData.platforms
    ...
    loadPlatformGrid() {
        ...
        container.innerHTML = wilmingtonDashboardData.platforms
    ...
    loadTopHashtags() {
        ...
        const topHashtags = wilmingtonDashboardData.getTopHashtags(15);
    ...
    createHashtagCategoryChart() {
        ...
        const analytics = wilmingtonDashboardData.getHashtagAnalytics();
    ...
    loadPostingSchedule() {
        ...
        const schedules = {
            'Business Hours': ['#WilmingtonFoods', '#FoodSafety', '#FoodTechnology'],
            'Peak Hours': ['#FoodQuality', '#SustainableFood', '#HealthyEating'],
            'Morning': ['#OrganicFoods', '#FreshProduce', '#NutritionFacts'],
            'Peak Times': ['#LocallySourced', '#NonGMO', '#FoodExcellence']
        };
    ...
    loadSuitabilityMatrix() {
        ...
        const platforms = ['All platforms', 'LinkedIn, Twitter', 'Instagram, Facebook', 'YouTube, Medium'];
        const suitabilityData = platforms.map(platform => {
            const count = wilmingtonDashboardData.hashtags.filter(h => 
    ...
    loadCompetitorComparison() {
        ...
        const activePlatforms = wilmingtonDashboardData.getPlatformStats().active;
        const totalPlatforms = wilmingtonDashboardData.getPlatformStats().total;
        ...
        container.innerHTML = `
            <div style="text-align: center; padding: 2rem;">
                ...
                <div style="margin-top: 2rem; display: grid; grid-template-columns: repeat(3, 1fr); gap: 2rem; text-align: center;">
                    ...
                    <div>
                        <div style="font-size: 2rem; color: var(--accent-danger);">${wilmingtonDashboardData.getPlatformStats().inactive}</div>
                        <div style="color: var(--text-secondary);">Inactive Platforms</div>
                    </div>
                    <div>
                        <div style="font-size: 2rem; color: var(--accent-primary);">${wilmingtonDashboardData.competitors.length}</div>
    ...
    loadGapAnalysis() {
        ...
        const inactivePlatforms = wilmingtonDashboardData.getPlatformsByStatus('Not Active');
    ...
    loadCompetitorList() {
        ...
        container.innerHTML = wilmingtonDashboardData.competitors.map(competitor => `
    ...
    loadPriorityPlatforms() {
        ...
        const recommendations = wilmingtonDashboardData.getInactivePlatformsWithPotential();
    ...
    loadPotentialHashtags() {
        ...
        const topHashtags = wilmingtonDashboardData.hashtags
    ...
    loadContentGaps() {
        ...
        const gaps = [
            { title: 'Google Business Profile', desc: '100 posts committed but platform not activated', priority: 'high' },
            { title: 'Pinterest Content Strategy', desc: '180 pieces committed but platform inactive', priority: 'high' },
            { title: 'YouTube Video Content', desc: 'Video strategy available but platform not active', priority: 'medium' },
            { title: 'Medium Thought Leadership', desc: '30 articles committed but platform not activated', priority: 'medium' }
        ];
    ...
    loadActionItems() {
        ...
        const actions = [
            { title: 'Activate Google Business Profile', desc: '100 posts committed - immediate local visibility impact', priority: 'high' },
            { title: 'Launch Pinterest Strategy', desc: '180 visual content pieces committed - high engagement potential', priority: 'high' },
            { title: 'Develop YouTube Channel', desc: 'Video content folder ready - great for food demonstrations', priority: 'medium' },
            { title: 'Activate Medium Publishing', desc: '30 thought leadership articles committed', priority: 'medium' }
        ];
...
// Initialize Wilmington dashboard when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
    new WilmingtonDashboard();
});