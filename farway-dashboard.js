// Farway Dashboard JavaScript - Updated for Farway CSV data
class FarwayDashboard {
    constructor() {
        this.charts = {};
    ...
    waitForData() {
        return new Promise((resolve) => {
            const checkData = () => {
                if (farwayDashboardData && farwayDashboardData.processedData) {
                    resolve();
                } else {
    ...
    updateHeaderMetrics() {
        const stats = farwayDashboardData.getPlatformStats();
        const contentMetrics = farwayDashboardData.getContentMetrics();
    ...
    loadContentPipeline() {
        ...
        const topPlatforms = farwayDashboardData.platforms
    ...
    loadPlatformGrid() {
        ...
        container.innerHTML = farwayDashboardData.platforms
            ...
            .map(platform => {
                ...
                // Format the content created link - Farway specific links
    ...
    loadTopHashtags() {
        ...
        const topHashtags = farwayDashboardData.getTopHashtags(15);
    ...
    createHashtagCategoryChart() {
        ...
        const analytics = farwayDashboardData.getHashtagAnalytics();
    ...
    loadSuitabilityMatrix() {
        ...
        const suitabilityData = platforms.map(platform => {
            const count = farwayDashboardData.hashtags.filter(h => 
    ...
    loadCompetitorComparison() {
        ...
        const activePlatforms = farwayDashboardData.getPlatformStats().active;
        const totalPlatforms = farwayDashboardData.getPlatformStats().total;
        ...
        container.innerHTML = `
            <div style="text-align: center; padding: 2rem;">
                ...
                <div style="margin-top: 2rem; display: grid; grid-template-columns: repeat(3, 1fr); gap: 2rem; text-align: center;">
                    ...
                    <div>
                        <div style="font-size: 2rem; color: var(--accent-danger);">${farwayDashboardData.getPlatformStats().inactive}</div>
                        <div style="color: var(--text-secondary);">Inactive Platforms</div>
                    </div>
                    <div>
                        <div style="font-size: 2rem; color: var(--accent-primary);">${farwayDashboardData.competitors.length}</div>
    ...
    loadGapAnalysis() {
        ...
        const inactivePlatforms = farwayDashboardData.getPlatformsByStatus('Not Active');
    ...
    loadCompetitorList() {
        ...
        container.innerHTML = farwayDashboardData.competitors.map(competitor => `
    ...
    loadPriorityPlatforms() {
        ...
        const recommendations = farwayDashboardData.getInactivePlatformsWithPotential();
    ...
    loadPotentialHashtags() {
        ...
        const topHashtags = farwayDashboardData.hashtags
    ...
    loadContentGaps() {
        ...
        const gaps = [
            { title: 'Blog Content Documentation', desc: 'Website-blogs has Google Docs link with 400 content pieces planned', priority: 'high' },
            { title: 'Google Business Setup', desc: 'Not available but has content plan documentation ready', priority: 'high' },
            { title: 'Video Platform Activation', desc: 'YouTube (15), TikTok (15) ready but platforms inactive', priority: 'medium' },
            { title: 'Professional Network Optimization', desc: 'LinkedIn active but missing content documentation', priority: 'medium' }
        ];
    ...
    loadActionItems() {
        ...
        const actions = [
            { title: 'Leverage Blog Content Strategy', desc: 'Website-blogs has detailed content plan (400 pieces) - prioritize execution', priority: 'high' },
            { title: 'Activate Google Business', desc: 'Content plan ready, just need to set up the platform presence', priority: 'high' },
            { title: 'Complete LinkedIn Documentation', desc: 'Active platform but missing Google Docs content strategy link', priority: 'medium' },
            { title: 'Expand Video Content', desc: 'YouTube and TikTok have content committed but platforms need activation', priority: 'low' }
        ];
...
// Initialize Farway dashboard when DOM is loaded
